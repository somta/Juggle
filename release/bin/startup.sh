...
JAVA_OPT="${JAVA_OPT} -Djuggle.home=${BASE_DIR}"
...
if [[ "$JAVA_OPT_EXT_FIX" == "" ]]; then
  nohup "$JAVA" ${JAVA_OPT} juggle.juggle >> ${BASE_DIR}/logs/start.out 2>&1 &
else
  nohup "$JAVA" "$JAVA_OPT_EXT_FIX" ${JAVA_OPT} juggle.juggle >> ${BASE_DIR}/logs/start.out 2>&1 &
fi

echo "juggle is starting，you can check the ${BASE_DIR}/logs/start.out"
