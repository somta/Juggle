...
permalink: /docs/guide/integration/open-api.html
---
...
| juggleToken | String | 必填     | OpenApi的令牌值，通过参数传递                                |
...
下面以系统内置示例流程为例，展示使用get的方式调用流程

```
http://localhost:9127/open/v1/flow/trigger/v1/sync_example?juggleToken=eyJ1c2VySWQiOjEsInRpbWVzdGFtcCI6MTcxMjE2MTIxNDAzOH0=&userName=juggle&password=123456&deposit=1000.00
```

![触发流程](images/openapi_trigger_flow_get.png)
...
| 名称         | 类型   | 是否必填 | 描述                            |
| ------------ | ------ | -------- | ------------------------------- |
| Juggle-Token | String | 非必填   | OpenApi的令牌值，通过请求头传递 |
...
下面以系统内置示例流程为例，展示通过post的方式调用流程
![触发流程](images/openapi_trigger_flow_post.png)
